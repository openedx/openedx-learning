...
def get_object_tags(
    ...
    if not include_deleted:
        base_qs = base_qs.exclude(taxonomy_id='')  # Exclude if the whole taxonomy was deleted
        base_qs = base_qs.exclude(tag_id='', taxonomy__allow_free_text=False)  # Exclude if just the tag is deleted
    tags = (
        base_qs
