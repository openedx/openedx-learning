...
def create_zip_file(lp_label: str, path: str, user: UserType | None = None, origin_server: str | None = None) -> None:
    """
    ...
    """
    learning_package = get_learning_package_by_key(lp_label)
    LearningPackageZipper(learning_package, user, origin_server).create_zip(path)


def load_learning_package(path: str, label: str | None = None, user: UserType | None = None) -> dict:
    """
    ...
    with zipfile.ZipFile(path, "r") as zipf:
        return LearningPackageUnzipper(zipf, label, user).load()
